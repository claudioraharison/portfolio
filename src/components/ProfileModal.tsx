...
const ProfileModal: React.FC<ProfileModalProps> = ({ isOpen, onClose }) => {
  ...
  // MODIFICATION: Navigation vers la page projets
  const handleViewProjects = () => {
    onClose(); // Ferme la modale
    navigate('/projects'); // Navigue vers la page des projets
  };
      ...
      >
        ...
        >
          ...
          <div className="border-t border-gray-200 bg-gray-50 p-3 sm:p-4 flex-shrink-0">
            <div className="flex flex-row gap-1.5 sm:gap-3 items-center justify-between">
              
              {/* Bouton Télécharger CV */}
              <button
                onClick={handleCvDownload}
                className="flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-2.5 sm:py-3 bg-gradient-to-r from-blue-950 to-blue-900 text-white rounded-lg font-semibold hover:from-blue-900 hover:to-blue-950 transition-all shadow-lg hover:shadow-xl flex-1 min-h-[44px] sm:min-h-[52px] group min-w-0"
                title={downloadCv}
              >
                <Download size={16} className="sm:size-[18px] flex-shrink-0" />
                <span className="text-xs sm:text-sm truncate hidden sm:inline">{downloadCv}</span>
                <span className="text-xs font-medium truncate sm:hidden">CV</span>
              </button>
              
              {/* Bouton Voir Projets */}
              <button
                onClick={handleViewProjects}
                className="flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-2.5 sm:py-3 border-2 border-blue-900 text-blue-900 rounded-lg font-semibold hover:bg-blue-900 hover:text-white transition-all flex-1 min-h-[44px] sm:min-h-[52px] group min-w-0"
                title={viewProjects}
              >
                <Eye size={16} className="sm:size-[18px] flex-shrink-0" />
                <span className="text-xs sm:text-sm truncate hidden sm:inline">{viewProjects}</span>
                <span className="text-xs font-medium truncate sm:hidden">Projets</span>
              </button>
              
              {/* Bouton Contact Direct */}
              <a
                href="mailto:claudio.raharison@gmail.com"
                className="flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-2.5 sm:py-3 bg-gradient-to-r from-blue-900 to-blue-950 text-white rounded-lg font-semibold hover:from-blue-950 hover:to-blue-900 transition-all shadow-lg hover:shadow-xl flex-1 min-h-[44px] sm:min-h-[52px] group min-w-0"
                title={contactDirect}
              >
                <Mail size={16} className="sm:size-[18px] flex-shrink-0" />
                <span className="text-xs sm:text-sm truncate hidden sm:inline">{contactDirect}</span>
                <span className="text-xs font-medium truncate sm:hidden">Contact</span>
              </a>
